...
const DB = (() => {
  ...
  function _ensureShape() {
    ...
    if (!Array.isArray(_data.library.pots))         _data.library.pots         = [];
    if (!Array.isArray(_data.library.lineageNodes)) _data.library.lineageNodes = [];
    if (typeof _data.onboardingDone === 'undefined') _data.onboardingDone = false;
  }
  ...
  function setOnboardingDone() { _data.onboardingDone = true; save(); }
  function isOnboardingDone()  { return !!_data.onboardingDone; }

  return { load, save, get, getPlant, getLib, upsertPlant, deletePlant, upsertLibItem, deleteLibItem, replaceFull, setOnboardingDone, isOnboardingDone };
})();
...
const Library = (() => {
  ...
  function renderScreen() {
    ...
    const potEl = document.getElementById('lib-pots-list');
    if (potEl) potEl.innerHTML = (lib.pots||[]).length
      ? (lib.pots||[]).map(p => _itemHTML('pots', p, _potIcon(p.type), `${p.volume}L · ${_potTypeLabel(p.type)}`)).join('')
      : '<div class="empty-hint">Nenhum vaso cadastrado.</div>';
    const linEl = document.getElementById('lib-lineage-list');
    if (linEl) linEl.innerHTML = (lib.lineageNodes||[]).length
      ? (lib.lineageNodes||[]).map(n => _itemHTML('lineageNodes', n, '🧬', `${n.generation||''}${n.seedbank?' · '+n.seedbank:''}`)).join('')
      : '<div class="empty-hint">Nenhuma genética cadastrada.</div>';
  }

  function _potIcon(type) {
    return { plastico:'🪣', tecido:'🧺', airpot:'⭕', inteligente:'🤖' }[type] || '🪴';
  }
  function _potTypeLabel(type) {
    return { plastico:'Plástico', tecido:'Tecido', airpot:'Airpot', inteligente:'Vaso inteligente' }[type] || type || '—';
  }
  ...
  function openItem(cat, id) {
    ...
    ['lib-item-name','lib-notes-substrate','lib-notes-recipe','lib-notes-nutrient','lib-item-recipe','lib-item-brand',
     ...
     'lib-pot-volume','lib-lineage-parent1','lib-lineage-parent2','lib-lineage-seedbank','lib-lineage-notes',
    ].forEach(i=>{ const el=document.getElementById(i); if(el) el.value=''; });
    UI.setPill('lib-soil-type','inerte');
    UI.setPill('lib-nut-type','mineral');
    UI.setPill('lib-pot-type','plastico');
    UI.setPill('lib-lineage-gen','F1');
    _updateLibFields(cat);
    if (item) {
      const sv=(id,v)=>{ const el=document.getElementById(id); if(el&&v!=null) el.value=v; };
      sv('lib-item-name',item.name);
      sv('lib-notes-substrate',item.notes); sv('lib-notes-recipe',item.notes); sv('lib-notes-nutrient',item.notes);
      ...
      if(cat==='setups') {
        sv('lib-setup-led-model',item.ledModel); sv('lib-setup-led-type',item.ledType);
        sv('lib-setup-led-watts',item.ledWatts); sv('lib-setup-photo-on',item.photoOn);
        sv('lib-setup-photo-off',item.photoOff); sv('lib-setup-tent-size',item.tentSize);
        sv('lib-setup-exhaust',item.exhaust); sv('lib-setup-fan',item.fan);
        sv('lib-setup-timer',item.timer); sv('lib-setup-co2',item.co2);
        sv('lib-setup-vpd-sensor',item.vpdSensor); sv('lib-setup-light-sensor',item.lightSensor);
        sv('lib-setup-camera',item.camera); sv('lib-setup-irrigation',item.irrigationAuto);
        sv('lib-setup-smart-plug',item.smartPlug);
      }
      if(cat==='pots') {
        sv('lib-pot-volume',item.volume);
        if(item.type) UI.setPill('lib-pot-type',item.type);
      }
      if(cat==='lineageNodes') {
        sv('lib-lineage-parent1',item.parent1); sv('lib-lineage-parent2',item.parent2);
        sv('lib-lineage-seedbank',item.seedbank); sv('lib-lineage-notes',item.notes);
        if(item.generation) UI.setPill('lib-lineage-gen',item.generation);
      }
    }
  ...
  function _updateLibFields(cat) {
    ['lib-fields-substrate','lib-fields-recipe','lib-fields-nutrient','lib-fields-setup','lib-fields-pot','lib-fields-lineage']
      .forEach(id=>{ const el=document.getElementById(id); if(el) el.classList.add('hidden'); });
    const map={ substrates:'lib-fields-substrate', recipes:'lib-fields-recipe', nutrients:'lib-fields-nutrient', setups:'lib-fields-setup', pots:'lib-fields-pot', lineageNodes:'lib-fields-lineage' };
    const el=document.getElementById(map[cat]); if(el) el.classList.remove('hidden');
    // nome label
    const nameLabel = document.querySelector('label[for="lib-item-name"]');
    if (nameLabel) nameLabel.textContent = cat==='lineageNodes' ? 'Nome da strain / cultivar' : cat==='pots' ? 'Nome do vaso (opcional)' : 'Nome';
  }

  function _catLabel(cat) {
    return { substrates:'Novo substrato', recipes:'Nova receita', nutrients:'Novo nutriente', setups:'Novo setup', pots:'Novo vaso', lineageNodes:'Nova genética' }[cat]||'Novo item';
  }

  function saveItem() {
    const nameEl = document.getElementById('lib-item-name');
    ...
    let item = { id: _editId || Utils.uid() };

    if (_editCat === 'pots') {
      const vol = nv('lib-pot-volume');
      if (!vol) { UI.toast('Informe o volume do vaso.'); return; }
      const potName = nameEl.value.trim() || `${_potTypeLabel(UI.getPill('lib-pot-type'))} ${vol}L`;
      item = { ...item, name:potName, volume:vol, type:UI.getPill('lib-pot-type') };
    } else if (_editCat === 'lineageNodes') {
      const name = nameEl.value.trim();
      if (!name) { UI.toast('Informe o nome da strain.'); return; }
      item = { ...item, name, generation:UI.getPill('lib-lineage-gen'),
        parent1:sv('lib-lineage-parent1'), parent2:sv('lib-lineage-parent2'),
        seedbank:sv('lib-lineage-seedbank'), notes:sv('lib-lineage-notes') };
    } else {
      const name = nameEl.value.trim();
      if (!name) { UI.toast('Informe o nome.'); return; }
      item.name = name;
      if (_editCat === 'substrates') {
        item = { ...item, type:UI.getPill('lib-soil-type'), brand:sv('lib-item-brand'), notes:sv('lib-notes-'+(_editCat==='substrates'?'substrate':_editCat==='recipes'?'recipe':_editCat==='nutrients'?'nutrient':'setup')) };
      } else if (_editCat === 'recipes') {
        item = { ...item, recipe:sv('lib-item-recipe'), notes:sv('lib-notes-'+(_editCat==='substrates'?'substrate':_editCat==='recipes'?'recipe':_editCat==='nutrients'?'nutrient':'setup')) };
      } else if (_editCat === 'nutrients') {
        item = { ...item, type:UI.getPill('lib-nut-type'), doseMin:nv('lib-item-dose-min'), doseMax:nv('lib-item-dose-max'), notes:sv('lib-notes-'+(_editCat==='substrates'?'substrate':_editCat==='recipes'?'recipe':_editCat==='nutrients'?'nutrient':'setup')) };
      } else if (_editCat === 'setups') {
        item = { ...item, ledModel:sv('lib-setup-led-model'), ledType:sv('lib-setup-led-type'),
          ledWatts:sv('lib-setup-led-watts'), photoOn:sv('lib-setup-photo-on'), photoOff:sv('lib-setup-photo-off'),
          tentSize:sv('lib-setup-tent-size'), exhaust:sv('lib-setup-exhaust'), fan:sv('lib-setup-fan'),
          timer:sv('lib-setup-timer'), co2:sv('lib-setup-co2'), vpdSensor:sv('lib-setup-vpd-sensor'),
          lightSensor:sv('lib-setup-light-sensor'), camera:sv('lib-setup-camera'),
          irrigationAuto:sv('lib-setup-irrigation'), smartPlug:sv('lib-setup-smart-plug'), notes:sv('lib-notes-'+(_editCat==='substrates'?'substrate':_editCat==='recipes'?'recipe':_editCat==='nutrients'?'nutrient':'setup')) };
      }
    }
...
const Genealogy = (() => {
  function render(activePlantId) {
    ...
    if (!el) return;
    const { plants } = DB.get();
    const plant = DB.getPlant(activePlantId);
    if (!plant) { el.innerHTML = '<div class="empty-hint">Planta não encontrada.</div>'; return; }

    // Clone tree (plantas cadastradas)
    const roots = plants.filter(p => !p.parentId);
    const cloneTree = roots.map(r => _nodeHTML(r, plants, activePlantId, 0)).join('');

    // Lineage genética
    const lineageBtn = plant.lineageNodeId
      ? `<button class="lineage-add-btn" onclick="Lineage.openGraph('${activePlantId}')">🧬 Ver linhagem genética</button>`
      : `<button class="lineage-add-btn" onclick="Lineage.openGraph('${activePlantId}')">🧬 Adicionar linhagem genética</button>`;

    el.innerHTML = `
      <div class="genealogy-wrap">
        ${lineageBtn}
        <div class="section-label" style="padding:12px 0 8px">Árvore de clones</div>
        <div class="gene-tree">${cloneTree || '<div class="empty-hint">Nenhuma relação de clone cadastrada.</div>'}</div>
      </div>`;
  }
...
/* ═══════════════════════════════════════════════
   LINEAGE — Gráfico de linhagem genética
═══════════════════════════════════════════════ */
const Lineage = (() => {
  let _activePlantId = null;

  const GEN_BADGE = { F1:'badge-f1', F2:'badge-f2', IBL:'badge-ibl', BX:'badge-bx', Landrace:'badge-landrace', Clone:'badge-clone' };

  function openGraph(plantId) {
    _activePlantId = plantId;
    const plant = DB.getPlant(plantId);
    document.getElementById('lineage-plant-name').textContent = plant ? plant.name : '—';
    _renderGraph();
    _renderNodeList();
    Modals.open('modal-lineage');
  }

  function _renderGraph() {
    const lib   = DB.getLib();
    const nodes = lib.lineageNodes || [];
    const plant = DB.getPlant(_activePlantId);
    const wrap  = document.getElementById('lineage-svg-container');
    if (!wrap) return;

    if (!nodes.length) {
      wrap.innerHTML = '<div class="lineage-empty">Nenhuma genética cadastrada na biblioteca.<br>Adicione em <strong>Biblioteca → 🧬 Genética</strong>.</div>';
      return;
    }

    // Layout: binary tree bottom-up
    // root = planta atual (ou strain principal), pais acima
    const rootId = plant?.lineageNodeId;
    const rootNode = rootId ? nodes.find(n => n.id === rootId) : null;

    if (!rootNode) {
      wrap.innerHTML = `<div class="lineage-empty">Nenhuma strain vinculada a esta planta.<br><button class="lineage-add-btn" style="margin-top:12px" onclick="Lineage.linkStrain()">🔗 Vincular strain</button></div>`;
      return;
    }

    // Build tree recursively resolving parent1/parent2 by name
    const findByName = name => nodes.find(n => n.name.toLowerCase() === (name||'').toLowerCase());
    const W=160, H=70, PAD=20, VGAP=90;

    function buildTree(node, depth, pos) {
      if (!node || depth > 4) return null;
      const p1 = node.parent1 ? findByName(node.parent1) : null;
      const p2 = node.parent2 ? findByName(node.parent2) : null;
      return { node, depth, pos, left: p1?buildTree(p1,depth+1,pos*2):null, right: p2?buildTree(p2,depth+1,pos*2+1):null };
    }

    function countLeaves(t) { if(!t) return 1; return countLeaves(t.left)+countLeaves(t.right); }
    function assignX(t, startX, availW) {
      if(!t) return; t.x = startX+availW/2;
      if(t.left&&t.right){ assignX(t.left,startX,availW/2); assignX(t.right,startX+availW/2,availW/2); }
      else if(t.left){ assignX(t.left,startX,availW); }
      else if(t.right){ assignX(t.right,startX,availW); }
    }

    const tree = buildTree(rootNode, 0, 0);
    const leaves = countLeaves(tree);
    const svgW   = Math.max(340, leaves * (W+PAD));
    assignX(tree, PAD, svgW - PAD*2);

    // Collect all nodes
    const allNodes=[]; const edges=[];
    function collect(t, parentX, parentY) {
      if(!t) return;
      const y = PAD + t.depth*VGAP;
      allNodes.push({...t, y});
      if(parentX!==null) edges.push({x1:parentX, y1:parentY+H/2, x2:t.x, y2:y+H/2});
      collect(t.left,  t.x, y);
      collect(t.right, t.x, y);
    }
    collect(tree, null, null);
    const svgH = PAD + (allNodes.reduce((m,n)=>Math.max(m,n.depth),0)+1)*VGAP + H + PAD;

    const edgesHTML = edges.map(e =>
      `<path class="lineage-edge" d="M${e.x1},${e.y1} C${e.x1},${(e.y1+e.y2)/2} ${e.x2},${(e.y1+e.y2)/2} ${e.x2},${e.y2}"/>`
    ).join('');

    const nodesHTML = allNodes.map(n => {
      const isRoot = n.node.id === rootId;
      const rectCls = isRoot ? 'lineage-node-rect is-active' : n.node.generation==='Landrace'?'lineage-node-rect is-landrace':'lineage-node-rect';
      const nameText = n.node.name.length>16 ? n.node.name.substring(0,14)+'…' : n.node.name;
      return `<g class="lineage-node-group" transform="translate(${n.x - W/2},${n.y})">
        <rect class="${rectCls}" width="${W}" height="${H}" rx="10" ry="10"/>
        <text class="lineage-node-name" x="${W/2}" y="26" text-anchor="middle">${Utils.esc(nameText)}</text>
        <text class="lineage-node-gen"  x="${W/2}" y="44" text-anchor="middle">${n.node.generation||''}</text>
        ${n.node.seedbank?`<text class="lineage-node-gen" x="${W/2}" y="58" text-anchor="middle">${Utils.esc(n.node.seedbank.substring(0,20))}</text>`:''}
      </g>`;
    }).join('');

    wrap.innerHTML = `<div class="lineage-svg-wrap" style="overflow-x:auto">
  ...
  function _renderNodeList() {
    ...
    const plant = DB.getPlant(_activePlantId);
    const nodes = lib.lineageNodes || [];
    const el    = document.getElementById('lineage-node-list');
    if (!el) return;
    el.innerHTML = nodes.map(n => {
      const isLinked = plant?.lineageNodeId === n.id;
      const badgeCls = GEN_BADGE[n.generation] || 'badge-f1';
      return `<div class="lineage-node-item ${isLinked?'root-node':''}">
        <div style="flex:1;min-width:0">
          <div style="font-size:14px;font-weight:700;color:var(--text-0)">${Utils.esc(n.name)}</div>
          <div style="font-size:11px;color:var(--text-3);margin-top:2px">${[n.parent1,n.parent2].filter(Boolean).join(' × ')||'Origem desconhecida'}${n.seedbank?' · '+n.seedbank:''}</div>
        </div>
        <span class="lineage-node-badge ${badgeCls}">${n.generation||'—'}</span>
        ${isLinked
          ? `<button class="btn ghost sm" onclick="Lineage.unlinkStrain()">Desvincular</button>`
          : `<button class="btn accent sm" onclick="Lineage.linkStrainId('${n.id}')">Vincular</button>`}
      </div>`;
    }).join('') || '<div class="empty-hint">Nenhuma genética na biblioteca.</div>';
  }

  function linkStrainId(nodeId) {
    const plant = DB.getPlant(_activePlantId);
    if (!plant) return;
    plant.lineageNodeId = nodeId;
    DB.upsertPlant(plant);
    _renderGraph();
    _renderNodeList();
    UI.toast('Strain vinculada! 🧬');
  }

  function unlinkStrain() {
    const plant = DB.getPlant(_activePlantId);
    if (!plant) return;
    delete plant.lineageNodeId;
    DB.upsertPlant(plant);
    _renderGraph();
    _renderNodeList();
    UI.toast('Strain desvinculada.');
  }

  function linkStrain() {
    // Abre a lista de nós para o usuário escolher
    _renderNodeList();
  }

  return { openGraph, linkStrainId, unlinkStrain, linkStrain };
})();
...
const Onboarding = (() => {
  const STEPS = [
    { id:'ob-step-welcome',  icon:'🌸', title:'Bem-vindo ao GrowLog', sub:'Seu diário de cultivo indoor. Antes de começar, vamos configurar sua biblioteca de insumos e equipamentos.' },
    { id:'ob-step-setup',    icon:'💡', title:'Seu setup de cultivo',  sub:'Cadastre sua tenda, LED, fotoperíodo, sensores e automações. Use o mesmo setup para todos os cultivos.' },
    { id:'ob-step-soil',     icon:'🪨', title:'Substrato e nutrientes', sub:'Cadastre seus substratos, receitas de solo e nutrientes com dosagem. Eles ficam disponíveis em todos os registros.' },
    { id:'ob-step-pots',     icon:'🪴', title:'Vasos', sub:'Cadastre os vasos que você usa — tipo e volume. Útil para acompanhar transplantes.' },
    { id:'ob-step-genetics', icon:'🧬', title:'Genética e linhagem', sub:'Cadastre as strains e sua linhagem genética. Opcional — você pode fazer isso depois.' },
    { id:'ob-step-done',     icon:'✅', title:'Tudo pronto!', sub:'Sua biblioteca está configurada. Agora é só adicionar sua primeira planta e começar a registrar.' },
  ];
  ...
  function _render() {
    ...
    document.querySelectorAll('.ob-dot').forEach((d,i) => {
      d.classList.toggle('done',   i < _step);
      d.classList.toggle('active', i === _step);
    });
    // steps
    document.querySelectorAll('.ob-step').forEach((s,i) => s.classList.toggle('active', i === _step));
    // footer buttons
    const footer = document.getElementById('ob-footer');
    const isLast = _step === STEPS.length - 1;
    const isFirst= _step === 0;
    footer.innerHTML = `
      ${isFirst ? '' : `<button class="btn ghost" onclick="Onboarding.prev()">Voltar</button>`}
      ${isLast
        ? `<button class="btn accent" onclick="Onboarding.finish()">Começar!</button>`
        : `<button class="btn accent" onclick="Onboarding.next()">${_step===0?'Vamos lá →':'Próximo →'}</button>`}`;
  }

  function next() { if (_step < STEPS.length-1) { _step++; _render(); } }
  function prev() { if (_step > 0) { _step--; _render(); } }
  function finish() {
    DB.setOnboardingDone();
    const el = document.getElementById('onboarding-overlay');
    if (el) { el.style.opacity='0'; setTimeout(()=>el.classList.add('hidden'),400); }
    Render.home();
    UI.toast('Tudo configurado! Adicione sua primeira planta. 🌱');
  }
...
(async function init() {
  ...
  ['mp-type-group','stage-pill-group','entry-stage-group','af-poda-type','af-lst-type',
   'hr-rating-group','lib-soil-type','lib-nut-type','lib-pot-type','lib-lineage-gen'].forEach(id => UI.initPills(id));
